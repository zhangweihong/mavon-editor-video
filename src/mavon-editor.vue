...
import "./lib/font/css/iconfont.css";
  ...
  methods: {
    ...
    $imgAdd(pos, $file, isinsert) {
      ...
      this.__oFReader.onload = function (oFREvent) {
         let localUrl = window.URL.createObjectURL(oFile);
        $vm.markdownIt.image_add(localUrl, oFREvent.target.result);
    ...
    createVideoHtml(url){
        return "<video src='"+url+"' controls style='max-height:300px;max-width:550px' disablePictureInPicture controlslist='nodownload'></video>";
    },
    $videoAdd($file) {
      if ($file == null) return;
      var $vm = this;
      var oFile = $file;
      let __filter = /^video\//i;
      let localUrl = ""
      if ($file) {
        if (__filter.test(oFile.type)) {
           localUrl = window.URL.createObjectURL(oFile);
        } else {
      ...
      $vm.insertText($vm.getTextareaDom(), {
        prefix: $vm.createVideoHtml(localUrl),
        subfix: "",
        str: "",
      });
      $vm.$nextTick(function () {
        $vm.$emit("videoAdd", $file, localUrl);
      });
    },
