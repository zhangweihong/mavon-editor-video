<template>
  <div class="v-left-item">
    ...
    <div
      :disabled="!editable"
      class="op-icon mavon-editor-iconfont mavon-editor-iconshipin dropdown dropdown-wrapper"
      @mouseleave="$mouseleave_video_dropdown"
      @mouseenter="$mouseenter_video_dropdown"
      type="button"
      :class="{ selected: s_video_dropdown_open }"
      aria-hidden="true"
    >
      <transition name="fade">
        <div
          class="op-image popup-dropdown"
          v-show="s_video_dropdown_open"
          @mouseleave="$mouseleave_video_dropdown"
          @mouseenter="$mouseenter_video_dropdown"
        >
          <div class="dropdown-item" style="overflow: hidden">
  ...
  methods: {
    ...
    $imgFileAdd($file) {
        if($file.size > this.imgSize * 1024 * 1024){
            alert($file.name + "不能大于 " + this.imgSize + " M");
            return
        }
    ...
    $videoAdd($e) {
      ...
      if (files != null && files.length > 0) {
          let file = files[0]
         if(file.size > this.videoSize * 1024 * 1024){
            alert(file.name + " 不能大于 " + this.videoSize + " M");
            return
        }
        this.$emit("videoAdd", files[0]);
      }
       this.s_video_dropdown_open = false;  
      $e.target.value = ""; // 初始化
    },
    ...
    $mouseleave_video_dropdown() {
      ...
      this.video_timer = setTimeout(function () {
        vm.s_video_dropdown_open = false;
      }, 200);
    },
    $mouseenter_video_dropdown() {
      if (this.editable) {
        clearTimeout(this.video_timer);
        this.s_video_dropdown_open = true;
      }
    },
